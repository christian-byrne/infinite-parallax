...
class Logger(LoggerInterface):
    ...
    def pad_with_rules(self, text: str, embed_text) -> str:
        return "\n".join(
            [self.get_embedded_hrule(embed_text), text, self.__get_hrule()]
        )
    ...
    def progress_bar(self, cur: str, total: str, header: str, caller_prefix: str):
        ...
        empty = progress_bar_charsets[self.PROGRESS_CHARSET]["empty"]

        # Decay cur and total if necessary to fit on one line
        ...
        empty_str = empty[cur % len(empty)]

        bar = filled_str * cur + empty_str * (total - cur)

        display = f"{prefix}{header} {bar} {original_cur: >2}/{original_total: <2} {original_cur/original_total*100:.2f}%".replace(
            "\n", ""
        )

        if original_cur == 1:
            print(" ")
        if original_cur == original_total:
            print(display)
            print(" ")
        else:
            print(display, end="\r")
    ...
    def get_embedded_hrule(self, embed_text) -> str:
        if not self.__time_has_changed():
            return self.__get_hrule()

        non_embedded_len = self.__len_without_ansi(self.__get_hrule())
    ...
    def __text_to_lines(self, text: str, caller_prefix) -> str:
    ...
    def __decolor(self, text: str) -> str:
    ...
    def __format_log_message(
        ...
        *args,
        ...
        header, text = self.__extract_header(in_string)
        lines_string = self.__text_to_lines(text, caller_prefix)
    ...
    def __extract_header(self, msg: str) -> list[str]:
